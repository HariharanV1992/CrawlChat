...
update_lambdas() {
    log "Updating all Lambda functions..."
    IMAGE_URI="$AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/$ECR_REPOSITORY:$IMAGE_TAG"
    for fn in crawlchat-api-function crawlchat-crawler-function crawlchat-crawl-worker; do
        log "Updating Lambda function: $fn"
        aws lambda update-function-code \
            --function-name "$fn" \
            --image-uri "$IMAGE_URI" \
            --region "$AWS_REGION"
        if [ $? -eq 0 ]; then
            success "Lambda function $fn updated successfully"
        else
            error "Failed to update Lambda function $fn"
            exit 1
        fi
        log "Waiting for Lambda function $fn update to complete..."
        aws lambda wait function-updated --function-name "$fn" --region "$AWS_REGION"
        success "Lambda function $fn update completed"
    done
}
...
main() {
    ...
    log "aws logs tail /aws/lambda/crawlchat-api-function --follow --region $AWS_REGION"
}
